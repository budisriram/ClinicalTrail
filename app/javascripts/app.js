...
import clinicalTrail from '../../build/contracts/ClinicalTrial.json';
...
// Initialize
function deployClinicalTrail() {
	
	//var clinicalTrailABI = contract(clinicalTrail);
	
	var clinicalTrailABI = contract(clinicalTrail);
	
	 clinicalTrailABI.setProvider(provider)
    clinicalTrailABI.defaults({from: currentAccount,gas:defaultGas})
	
	
	clinicalTrailABI.deployed().then(function(instance){
...
function deployRegulator() {
	...
	//var regulatorABI = artifacts.require("./Regulator.sol")
	var regulatorABI = contract(regulator);
	
	regulatorABI.setProvider(provider);
	
   regulatorABI.defaults({from: currentAccount,gas:defaultGas});
	
	
	regulatorABI.deployed().then(function(instance){
		regulatorInstance = instance;
		regulatorContractAddress = regulatorInstance.address;
		$('#sectionAAddress').html('<i class="fa fa-address-card"></i> ' + regulatorInstance.address);
            $('#sectionATxnHash').html('<i class="fa fa-list-alt"></i> ' + regulatorInstance.transactionHash);
...
function initializeClinicalTrialEvents() {
   ...
   // clinicalTrailABI.defaults({from: currentAccount,gas:defaultGas})
	...
	clinicalTrailABI.deployed().then(function(clinicalTrailInstance){
		ct = clinicalTrailABI.at(ctContractAddress);
	});
...
function addSubjects() {
   ...
   // clinicalTrailABI.defaults({from: currentAccount,gas:defaultGas})
	...
	clinicalTrailABI.deployed().then(function(clinicalTrailInstance){
		 var ct = clinicalTrailABI.at(ctContractAddress);
    ...
    function addSubjectTransaction(sub, currentValue) {
        ...
        ct.addSubject.sendTransaction(sub, { from: croAccount, gas: defaultGas }).then(function(txHash) {
...
function addDataPoints() {
   ...
   // clinicalTrailABI.defaults({from: currentAccount,gas:defaultGas})
    ...
    function addDataTransaction(_subjId, _json) {
        var deferred = Q.defer();
        //croAccount
        ct.addDataPoint.sendTransaction(_subjId, _json, { from: croAccount, gas: defaultGas }).then(function(txHash) {
...
function getBlockDetails(get) {
	var blockNo = get.innerHTML;
...
function getBlockInfo() {
    ...
    blocks.forEach(function(block) {
        for (var index = 0; index < block.transactions.length; index++) {
            ...
            $('#transactionsbody').append('<tr><td><button  target="#" onclick="getBlockDetails(this)">' + block.number + '</button></td><td>' + block.hash +
